...
import 'package:kejaksaan/screen_page/pegawai/detail_pegawai.dart';

import 'package:kejaksaan/screen_page/pegawai/page_pegawai.dart';
import 'package:kejaksaan/screen_page/pegawai/update_pegawai.dart';
...
class PageListPegawai extends StatefulWidget {
  final Datum? data;
  const PageListPegawai({this.data, super.key});
  ...
  Future<void> getPegawai() async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse(
          '$url/db_kejaksaan/listPengaduan.php?id_user=$idUser&kategori=pegawai'));
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (data.status == "success") {
          setState(() {
            pegawaiList.removeWhere((pegawai) => pegawai.id == id.toString());
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
              context, MaterialPageRoute(builder: (context) => PagePegawai()));
        },
