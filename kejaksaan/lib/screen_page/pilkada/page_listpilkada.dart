...
import 'package:kejaksaan/screen_page/pilkada/detail_pilkada.dart';
import 'package:kejaksaan/screen_page/pilkada/page_pilkada.dart';
import 'package:kejaksaan/screen_page/pilkada/update_pilkada.dart';
import 'package:kejaksaan/utils/ip.dart';
import 'package:shared_preferences/shared_preferences.dart';

class PageListPilkada extends StatefulWidget {
  final Datum? data;
  const PageListPilkada({this.data, super.key});
  ...
  Future<void> getPegawai() async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse(
          '$url/db_kejaksaan/listPengaduan.php?id_user=$idUser&kategori=pilkada'));
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (data.status == "success") {
          setState(() {
            pilkadaList.removeWhere((pegawai) => pegawai.id == id.toString());
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
              context, MaterialPageRoute(builder: (context) => PagePilkada()));
        },
        tooltip: "tambah posko",
        child: Icon(
          Icons.add,
          size: 25,
        ),
            ...
            child: isLoading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final pegawai = pilkadaList[index];
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  DetailPilkada(pengaduan: pegawai),
                            ),
                          );
                        },
                        child: Padding(
